<?php
class UNL_OpenMap_FilteredMarkerList extends IteratorIterator
{
    ...
    function __construct($options = array())
    {
        ...
        $list_class = 'UNL_OpenMap_MarkerList_'.$this->options['feature'];
        if (!class_exists($list_class)) {
            throw new Exception('Invalid marker list type');
        }

        $filter_class = 'UNL_OpenMap_MarkerList_Filter_'.$this->options['filter'];
