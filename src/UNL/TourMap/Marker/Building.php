<?php
class UNL_TourMap_Marker_Building extends UNL_TourMap_Marker
{
    public $campus;
    ...
    public function __construct($options = array())
    {
        ...
        if (isset($options['nugget']) && in_array($options['nugget'], array('info', 'image', 'video'))) {
            $this->info = new UNL_TourMap_Marker_Building_Info($options);
        }
    }

    function getInfo()
    {
        $this->info = new UNL_TourMap_Marker_Building_Info(get_object_vars($this));
        return $this->info;
    }
}