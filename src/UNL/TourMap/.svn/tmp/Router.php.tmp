...
class UNL_TourMap_Router
{
    public static function getRoute($requestURI)
    {
        ...
        $requestURI = substr($requestURI, strlen(parse_url(UNL_TourMap::getURL(), PHP_URL_PATH)));
    ...
    protected static function determineView($requestURI)
    {
       ...
       /*     case preg_match('/^images\/tilesets\/unl\/16\/15166\/([\w]+).([\w]+)$/', $requestURI, $matches):
                // Marker image that is unknown
                echo $matches[1];exit;
                header('Location: '.UNL_TourMap::getURL().'images/tilesets/'.$matches[0]);
                exit();
                break;*/
            default:
                $options['view'] = 404;
